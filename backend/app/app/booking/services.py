...
async def get_booking_by_id(booking_id: int,
                            db_session : Session) -> Booking:
    booking = db_session.query(Booking).get(booking_id)
    return booking

async def get_booking_by_idFlight(idFlight : int,
                                  db_session : Session) -> List[Booking]:
    bookings = db_session.query(Booking).filter(Booking.idFlight == idFlight).all()
    return bookings

async def get_all_bookings(db_session : Session,
                           customerName : str,
                           status : schema.BookingStatus) -> List[Booking]:
...
async def create_booking(idFlight : int,
                         idUser: int,
